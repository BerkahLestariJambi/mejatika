...
function NetworkLabContent() {
  ...
  const onDrop = useCallback((event: any) => {
    ...
    setNodes((nds) => nds.concat({
      ...
      data: { type, shapeType: val, label: val.toUpperCase(), status: 'up', onChange: (v: string) => onNodeLabelChange(id, v) },
      style: { width: 100, height: 100 }
    }));
  }, []);
          ...
          {activeTab === 'simulasi' ? (
            ...
            <div className="space-y-3">
              {curriculumMaterials.map((mat) => (
                <button key={mat.id} className="w-full p-4 rounded-xl border bg-white flex items-center justify-between hover:border-blue-300 transition-all text-left">
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.keys(iconLib).slice(0, 10).map(item => (
                <div key={item} draggable onDragStart={e => { e.dataTransfer.setData('application/type', 'device'); e.dataTransfer.setData('application/value', item); }} className="p-3 border rounded-xl flex flex-col items-center bg-white hover:bg-slate-50 cursor-grab shadow-sm transition-all hover:scale-105 active:scale-95">
      ...
      <div className="flex-grow flex flex-col relative" ref={reactFlowWrapper}>
        ...
        <div className="absolute top-4 left-4 z-[100] flex flex-col gap-2">
          ...
          <button 
            ...
            className={`w-full py-2 text-[10px] font-black rounded-xl shadow-lg border flex items-center justify-center gap-2 transition-all ${showDefinition ? 'bg-amber-500 text-white border-amber-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
          >
        ...
        {topologyInfo && showDefinition && (
          <div className="absolute bottom-24 left-6 z-[100] animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="bg-white/95 backdrop-blur-md p-5 rounded-2xl border border-blue-100 shadow-2xl w-[400px]">
            ...
            <div style={{ top: menu.y, left: menu.x }} className="fixed z-[1000] bg-white border shadow-2xl rounded-2xl p-4 min-w-[220px]">
              ...
              {!menu.isEdge ? (
                <div className="space-y-2">
                  ...
                  <button onClick={() => setNodes(nds => nds.filter(n => n.id !== menu.id))} className="w-full py-2 bg-red-50 text-red-600 text-[10px] font-bold rounded-lg uppercase">Delete Node</button>
                </div>
              ) : (
                <div className="space-y-2">
                    <button 
                    ...
                    <button onClick={() => setEdges(eds => eds.filter(e => e.id !== menu.id))} className="w-full py-2 bg-red-50 text-red-600 text-[10px] font-bold rounded-lg uppercase">Remove Link</button>
                </div>
              )}
            </div>
          )}
...
export default function NetworkLabEditor() { return <ReactFlowProvider><NetworkLabContent /></ReactFlowProvider>; }'use client';
