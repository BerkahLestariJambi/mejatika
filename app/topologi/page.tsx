...
const UniversalNode = ({ id, data }: any) => {
  ...
  // Efek animasi saat simulasi jalan
  const activeClass = data.isLive ? 'animate-pulse text-blue-600' : 'text-slate-700';
  ...
  return (
    <div className="relative group">
      ...
      <div className={`flex flex-col items-center justify-center min-w-[120px] p-4 bg-white border-[3px] rounded-xl shadow-xl transition-all ${isDown ? 'border-red-500 bg-red-50' : data.isLive ? 'border-blue-500 ring-4 ring-blue-100' : 'border-slate-800 hover:border-blue-500'}`}>
...
function NetworkLabContent() {
  ...
  // FUNGSI TOGGLE SIMULASI
  const toggleSimulation = () => {
    const nextState = !isLive;
    setIsLive(nextState);
    
    // Update semua node untuk efek visual
    setNodes((nds) => nds.map((n) => ({ ...n, data: { ...n.data, isLive: nextState } })));
    
    // Update semua kabel untuk animasi berjalan
    setEdges((eds) => eds.map((e) => ({ ...e, animated: nextState })));
  };
  ...
  const generateTopology = (type: 'bus' | 'mesh') => {
    ...
    if (type === 'bus') {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const isTop = i % 2 === 0;
        newNodes.push({ id: jId, type: 'universal', position: { x: xPos, y: backboneY }, data: { type: 'junction', isLive: false } });
        if (i > 0) {
          newEdges.push({ id: `back-${i}`, source: `j-${i-1}`, target: jId, style: { strokeWidth: 10, stroke: '#0f172a' }, type: 'straight', animated: false });
        }
        newNodes.push({ id: dId, type: 'universal', position: { x: xPos - 55, y: isTop ? backboneY - 200 : backboneY + 100 }, data: { shapeType: i === 0 ? 'router' : 'pc', label: i === 0 ? 'GATEWAY' : `PC-${i}`, onChange: updateNodeData, isLive: false } });
        newEdges.push({ id: `drop-${i}`, source: jId, target: dId, sourceHandle: isTop ? 'top' : 'bottom', targetHandle: isTop ? 'b' : 't', style: { strokeWidth: 4, stroke: '#0f172a' }, type: 'straight', animated: false });
      }
    } else {
        const mNodes = Array.from({ length: 5 }).map((_, i) => ({ id: `m-${i}`, type: 'universal', position: { x: 450 + 250 * Math.cos(2*Math.PI*i/5), y: 350 + 250 * Math.sin(2*PI*i/5) }, data: { shapeType: 'pc', label: `NODE-${i}`, onChange: updateNodeData, isLive: false } }));
        newNodes.push(...mNodes);
        for (let i = 0; i < 5; i++) for (let j = i + 1; j < 5; j++) newEdges.push({ id: `e-${i}-${j}`, source: `m-${i}`, target: `m-${j}`, style: { strokeWidth: 3, stroke: '#2563eb' }, animated: false });
    }
  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 overflow-hidden">
      <aside className="w-72 bg-white border-r flex flex-col z-[100] shadow-xl">
        ...
        <div className="flex bg-slate-50 text-[10px] font-black border-b">
          <button onClick={() => handleTabClick('inventory')} className={`flex-1 py-4 uppercase ${activeTab === 'inventory' ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-slate-400'}`}>Inventory</button>
          <button onClick={() => handleTabClick('simulasi')} className={`flex-1 py-4 uppercase ${activeTab === 'simulasi' ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-slate-400'}`}>Simulasi</button>
        </div>
          ...
          {activeTab === 'inventory' ? (
            ...
            <div className="space-y-4">
              <button 
                ...
                className={`w-full py-4 rounded-xl font-black text-xs shadow-lg flex items-center justify-center gap-3 transition-all active:scale-95 ${isLive ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
              >
              ...
              <div className="p-4 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                <p className="text-[9px] font-bold text-slate-400 uppercase leading-relaxed text-center">
                  {isLive ? "Aliran data sedang diproses dalam jaringan..." : "Klik tombol di atas untuk menguji konektivitas."}
                </p>
              </div>
            </div>
          )}
      ...
      <main className="flex-grow relative" ref={reactFlowWrapper}>
        {topologyType === 'praktek' && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.05]">
            <h1 className="text-8xl font-black uppercase text-slate-900 text-center leading-none">CANVAS<br/>PRAKTIK</h1>
          </div>
        )}
        ...
        <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onDrop={onDrop} onDragOver={(e) => e.preventDefault()} nodeTypes={nodeTypes} onConnect={(p) => setEdges(eds => addEdge({...p, style:{strokeWidth:4, stroke:'#0f172a'}, animated: isLive}, eds))} connectionMode={ConnectionMode.Loose} fitView>
        ...
        <div className={`absolute top-0 right-0 h-full flex z-[110] transition-transform duration-500 ${showPanel ? 'translate-x-0' : 'translate-x-[calc(100%-40px)]'}`}>
          <button onClick={() => setShowPanel(!showPanel)} className="h-full bg-slate-900 text-white w-10 flex flex-col items-center justify-center gap-4 border-l border-white/20">
          ...
          <div className="w-[340px] bg-white h-full p-8 shadow-2xl">
            <h2 className="text-xl font-black uppercase mb-6 flex items-center gap-3">
              <Info className="text-blue-600"/> Detail Lab
            </h2>
            <div className="space-y-4 text-xs font-bold text-slate-600 uppercase">
               <div className="p-4 bg-blue-50 rounded-lg">Status: <span className={isLive ? 'text-green-600' : 'text-red-600'}>{isLive ? 'LIVE' : 'IDLE'}</span></div>
               <div className="p-4 bg-slate-50 rounded-lg">Node: {nodes.length} Unit</div>
               <div className="p-4 bg-slate-50 rounded-lg">Kabel: {edges.length} Jalur</div>
            </div>
          </div>
        </div>
      </main>
