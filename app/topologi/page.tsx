...
function NetworkLabContent() {
  ...
  // Normalisasi Data agar tidak hilang (Jika Sidebar kirim format lama, kita ubah ke format kategori)
  const categoriesToRender = activeLesson?.categories || (activeLesson?.points ? [{ 
    name: "Materi Utama", 
    items: activeLesson.points.map((p: string) => ({
      title: p.includes(':') ? p.split(':')[0] : p,
      desc: p.includes(':') ? p.split(':')[1] : "Penjelasan detail mengenai komponen ini."
    }))
  }] : []);
                ...
                <div className="flex items-start gap-0 pointer-events-auto overflow-visible">
                  ...
                  <div className="bg-slate-900 border-2 border-blue-500 shadow-2xl rounded-2xl w-[240px] shrink-0 overflow-hidden animate-in fade-in zoom-in duration-300">
                    <div className="p-3 border-b border-white/10 flex justify-between items-center bg-blue-600/10">
                      <span className="text-[8px] font-black text-blue-400 uppercase italic tracking-widest">Pilih Kategori</span>
                      <button onClick={() => setActiveLesson(null)} className="text-white/50 hover:text-white"><X size={14}/></button>
                    </div>
                    <div className="p-4">
                      <h2 className="text-white text-sm font-black uppercase italic mb-1">{activeLesson.title}</h2>
                      <p className="text-[9px] text-slate-400 font-medium mb-4 leading-relaxed">{activeLesson.content}</p>
                      ...
                      <div className="space-y-2">
                        {categoriesToRender.map((cat: any, idx: number) => (
                          <button 
                            ...
                            className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all duration-200 ${activeCategory === cat.name ? 'bg-blue-600 border-blue-400 text-white shadow-[0_0_15px_rgba(37,99,235,0.4)]' : 'bg-white/5 border-white/10 text-slate-300 hover:bg-white/10'}`}
                          >
                            <span className="text-[10px] font-black uppercase italic">{cat.name}</span>
                            <ChevronRight size={12} className={activeCategory === cat.name ? "translate-x-1" : ""} />
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* LEVEL 2: LIST ITEM (KE KANAN) */}
                  {activeCategory && (
                    <>
                      <div className="w-10 h-[2px] bg-blue-500 shadow-[0_0_10px_blue] self-center shrink-0 animate-line-grow" />
                      <div className="flex flex-col gap-2 shrink-0">
                        {categoriesToRender.find((c: any) => c.name === activeCategory)?.items.map((item: any, i: number) => (
                          <div key={i} className="flex items-center">
                            ...
                            >
                              ...
                              <span className="text-[9px] font-black uppercase text-left leading-tight">{item.title}</span>
                            </button>
                          </div>
                        ))}
                      </div>
                  ...
                  {/* LEVEL 3: DETAIL LENGKAP (PALING KANAN) */}
                  {selectedSubPoint && (
                    <>
                      <div className="w-10 h-[2px] bg-blue-500 shadow-[0_0_10px_blue] self-center shrink-0 animate-line-grow" />
                      <div className="bg-white border-2 border-blue-600 shadow-2xl rounded-[2.5rem] w-[320px] shrink-0 overflow-hidden animate-in zoom-in-95 slide-in-from-left-10 duration-500">
                        <div className="h-32 bg-slate-50 flex items-center justify-center border-b border-slate-100 relative">
                           ...
                           <div className="bg-white p-5 rounded-3xl shadow-xl border border-blue-100 animate-bounce-subtle">
                              <Cpu size={40} className="text-blue-600" />
                           </div>
                        </div>
                        <div className="p-6">
                          <span className="bg-blue-100 text-blue-700 text-[8px] font-black px-2 py-0.5 rounded-full uppercase italic">Ensiklopedia IT</span>
