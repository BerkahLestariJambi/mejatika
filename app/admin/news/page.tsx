...
export default function NewsManagementPage() {
  ...
  // KONFIGURASI EDITOR (DITAMBAHKAN MODUL TABEL)
    ...
    if (file) {
      if (file.size > 2 * 1024 * 1024) { // 2MB
        Toast.fire({
          ...
          text: 'Ukuran gambar maksimal adalah 2MB bos!'
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Toast.fire({ icon: 'error', title: errData.message || 'Gagal menyimpan data' });
      }
    } catch (error) {
            ...
            <div className="bg-[#fffdfa] dark:bg-zinc-950 -mt-6 pt-12 pb-10 px-8 md:px-12 rounded-b-xl shadow-2xl relative border-x border-black/5 max-h-[85vh] overflow-y-auto custom-scrollbar">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase tracking-widest text-amber-600">Judul Berita</Label>
                  <Input 
                    value={title} 
                    onChange={(e) => setTitle(e.target.value)} 
                    required 
                    placeholder="Masukkan judul utama..." 
                    className="text-[18px] font-black uppercase tracking-wider border-amber-200 focus:ring-amber-500 bg-white"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase tracking-widest">Gambar Utama (Maks 2MB)</Label>
                  ...
                  <Input type="file" accept="image/*" onChange={handleImageChange} className="cursor-pointer file:bg-amber-50 file:text-amber-700 file:border-none bg-white" />
                </div>

                {/* AREA TEXT EDITOR DENGAN SCROLL */}
                <div className="space-y-2">
                  ...
                  <div className="bg-white rounded-md border border-amber-200 overflow-hidden">
                    <div className="max-h-[400px] overflow-y-auto"> {/* WRAPPER SCROLL DISINI */}
                      <ReactQuill 
                        theme="snow" 
                        value={content} 
                        onChange={setContent} 
                  ...
                  <p className="text-[9px] text-muted-foreground italic">*Gunakan toolbar untuk memasukkan tabel atau format lainnya.</p>
                </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                  ...
                  <tr key={item.id} className="border-b border-zinc-50 hover:bg-amber-50/30 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <img src={item.image || "https://placehold.co/600x400?text=No+Image"} className="h-10 w-14 object-cover rounded shadow-sm" alt="" />
                        <div>
                          <p className="font-bold text-sm text-zinc-800 line-clamp-1 group-hover:text-amber-600">{item.title}</p>
                          <p className="text-[9px] uppercase tracking-tighter text-muted-foreground italic text-nowrap">
                            Update: {item.created_at ? new Date(item.created_at).toLocaleDateString('id-ID') : '-'}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <Badge className="bg-amber-100 text-amber-700 text-[9px] border-none">{item.category?.name || 'Umum'}</Badge>
                    </td>
                ...
                {filteredNews.length === 0 && (
                  <tr>
                    <td colSpan={3} className="text-center py-10 text-muted-foreground text-sm italic">Data berita tidak ditemukan.</td>
                  </tr>
                )}
              </tbody>
            </table>
