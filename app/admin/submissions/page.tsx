...
export default function SubmissionsPage() {
  ...
  // 2. Kirim Feedback & Skor (Memicu Sertifikat di Backend)
  const submitFeedback = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Notifikasi cerdas: Jika nilai >= 75, beri tahu sertifikat terbit
    ...
    <div className="p-6 space-y-8 max-w-7xl mx-auto min-h-screen bg-zinc-50 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          ...
          <Button onClick={fetchData} variant="outline" className="h-12 w-12 rounded-2xl bg-white border-zinc-200 hover:bg-zinc-50">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 space-y-4">
          ...
          <p className="text-zinc-400 font-bold uppercase italic tracking-widest text-[10px]">Menghubungkan ke Mejatika API...</p>
        </div>
      ) : (
          ...
          <AnimatePresence mode="popLayout">
            {filteredSubmissions.map((sub) => (
              <motion.div key={sub.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}>
                <Card className="border-none shadow-md rounded-[2.5rem] overflow-hidden bg-white hover:shadow-xl transition-all group border border-transparent hover:border-amber-200">
                  <CardHeader className="bg-zinc-900 text-white p-6 group-hover:bg-zinc-800 transition-colors">
                    ...
                    <CardTitle className="text-lg font-black italic uppercase leading-tight tracking-tighter min-h-[2.5rem] line-clamp-2">
                      {sub.material?.title}
                    </CardTitle>
                  ...
                  <CardContent className="p-6 space-y-4">
                    <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100 relative group/box">
                      <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-1">Link / Jawaban Siswa:</span>
                    ...
                    <div className="flex items-center justify-between pt-2">
                       <div className="flex flex-col">
                         <span className="text-[8px] font-black text-zinc-400 uppercase">Status Penilaian</span>
                         <div className="flex items-center gap-1">
                           <span className={`text-sm font-black ${sub.score >= 75 ? 'text-emerald-500' : sub.score > 0 ? 'text-amber-500' : 'text-rose-500'}`}>
                             {sub.score > 0 ? `${sub.score}/100` : "PENDING"}
                           </span>
                           {sub.score >= 75 && <CheckCircle2 size={12} className="text-emerald-500" />}
                         </div>
                       </div>
                       
                       <Button 
                        onClick={() => handleOpenReview(sub)}
                        className={`rounded-xl font-black text-[10px] uppercase italic h-9 px-4 transition-all ${
        ...
        <DialogContent className="max-w-2xl rounded-[3rem] p-0 overflow-hidden border-none shadow-2xl bg-white">
          <div className="bg-zinc-900 p-8 text-white">
            <DialogHeader>
              <div className="flex items-center gap-2 mb-2">
                <Badge className="bg-amber-500 text-white border-none text-[9px] font-black">TAHAP EVALUASI</Badge>
              </div>
          ...
          <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-4">
               <Label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Diskusi & Progres</Label>
               
               {/* Bubble Chat: Tugas Awal */}
               <div className="flex flex-col items-start space-y-1">
                  <div className="bg-zinc-100 p-4 rounded-3xl rounded-tl-none border border-zinc-200 max-w-[90%]">
                     <p className="text-[9px] font-black text-zinc-400 uppercase mb-1">Lampiran Tugas Siswa:</p>
                     <p className="text-sm text-zinc-700 font-medium italic">"{selectedSub?.student_answer}"</p>
                     {selectedSub?.project_link && (
                        <a href={selectedSub.project_link} target="_blank" className="mt-2 block text-xs font-bold text-indigo-600">Lihat Link Project →</a>
                     )}
                  </div>
               </div>

               {/* Bubble Chat: Mentor Previous Feedback */}
               {selectedSub?.mentor_feedback && (
                 <div className="flex flex-col items-end space-y-1">
                    <div className="bg-zinc-900 p-4 rounded-3xl rounded-tr-none text-white max-w-[90%] shadow-lg">
               ...
               {/* Bubble Chat: Balasan Siswa */}
               {selectedSub?.student_reply && (
                 <div className="flex flex-col items-start space-y-1 animate-in fade-in slide-in-from-left-4">
                    <div className="bg-rose-500 p-4 rounded-3xl rounded-tl-none text-white max-w-[90%] shadow-md">
                       <p className="text-[9px] font-black text-rose-200 uppercase mb-1 flex items-center gap-1">
                         <MessageCircle size={10} /> Balasan Siswa:
                       </p>
            ...
            <div className="grid grid-cols-4 gap-4 bg-zinc-50 p-6 rounded-[2rem] border border-zinc-100">
              <div className="col-span-1 space-y-2">
                <Label className="font-black text-zinc-900 uppercase italic text-[10px]">Skor (0-100)</Label>
                <Input 
                  type="number" 
                  value={feedbackData.score}
                  onChange={(e) => setFeedbackData({...feedbackData, score: parseInt(e.target.value) || 0})}
                  className="h-12 rounded-2xl border-zinc-200 text-xl font-black text-center focus:border-amber-500"
                />
              </div>
              <div className="col-span-3 space-y-2">
                <Label className="font-black text-zinc-900 uppercase italic text-[10px]">Input Feedback / Jawaban Chat</Label>
                <textarea 
                  className="w-full h-24 p-4 rounded-2xl border border-zinc-200 focus:ring-2 focus:ring-amber-500 outline-none text-sm font-medium resize-none transition-all bg-white shadow-inner"
                  placeholder="Beri arahan atau apresiasi..."
            ...
            <div className="flex gap-3">
              <Button 
                ...
                className="flex-1 h-12 rounded-2xl font-black uppercase italic text-zinc-400"
              >
                Kembali
              </Button>
              <Button 
                ...
                disabled={isUpdating}
                className="flex-[2] h-12 bg-amber-500 hover:bg-zinc-900 text-white rounded-2xl font-black uppercase italic tracking-widest shadow-lg shadow-amber-200 transition-all flex items-center justify-center gap-2"
              >
