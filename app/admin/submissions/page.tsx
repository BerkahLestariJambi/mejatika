...
export default function SubmissionsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Mapping data Laravel agar konsisten
  ...
  // 2. Filter Pencarian (Aman dari null)
    ...
    <div className="p-6 space-y-8 max-w-7xl mx-auto min-h-screen bg-zinc-50 font-sans text-zinc-900">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <div className="relative flex-1 md:w-72">
            ...
            <Input 
              ...
              className="pl-10 rounded-2xl bg-white border-zinc-200 h-12" 
              value={search}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32">
          ...
          <p className="text-zinc-400 font-bold text-xs uppercase italic">Syncing...</p>
        </div>
      ) : (
          ...
          <AnimatePresence mode="popLayout">
            {filteredSubmissions.length > 0 ? (
                  ...
                  <Card className="border-none shadow-lg rounded-[2.5rem] overflow-hidden bg-white hover:shadow-2xl transition-all border border-zinc-100">
                    ...
                    <CardContent className="p-6 space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                         ...
                         }} className="rounded-xl bg-zinc-900 hover:bg-amber-500 font-black text-[10px] uppercase text-white h-9 px-4">
                           Review
                         </Button>
                      </div>
                    </CardContent>
              ...
              <div className="col-span-full py-20 text-center bg-white rounded-[3rem] border-2 border-dashed border-zinc-200">
                ...
                <p className="text-zinc-400 font-black uppercase italic">Belum ada data masuk</p>
              </div>
            )}
          </AnimatePresence>
        </div>
      ...
      <Dialog open={!!selectedSub} onOpenChange={() => setSelectedSub(null)}>
        <DialogContent className="max-w-2xl rounded-[3rem] p-0 overflow-hidden border-none shadow-2xl bg-white">
          ...
          <div ref={scrollRef} className="p-8 space-y-6 h-[350px] overflow-y-auto bg-zinc-50/50 flex flex-col">
            <div className="bg-white p-5 rounded-[2rem] rounded-tl-none border border-zinc-200 shadow-sm max-w-[90%]">
              ...
              {selectedSub?.project_link && (
                <a href={selectedSub.project_link} target="_blank" className="inline-flex items-center gap-1 text-[10px] text-blue-500 font-black underline">
            ...
            {selectedSub?.discussions?.map((chat: any) => (
              <div key={chat.id} className={`flex flex-col ${chat.user?.role === 'admin' ? 'items-end' : 'items-start'} space-y-1`}>
                <div className={`p-4 rounded-2xl max-w-[80%] ${chat.user?.role === 'admin' ? 'bg-zinc-900 text-white rounded-tr-none' : 'bg-white text-zinc-800 border border-zinc-200 rounded-tl-none'}`}>
          ...
          <div className="p-8 bg-white border-t border-zinc-100 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input type="number" value={feedbackData.score} onChange={(e) => setFeedbackData({...feedbackData, score: parseInt(e.target.value) || 0})} className="h-10 rounded-xl font-black text-lg w-24 text-center" />
               ...
               <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input placeholder="Tulis masukan..." value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendNewMessage()} className="h-10 rounded-xl bg-zinc-50 text-sm" />
            ...
            <Button variant="ghost" onClick={() => setSelectedSub(null)} className="w-full text-[10px] font-black uppercase text-zinc-400">Tutup Panel</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
