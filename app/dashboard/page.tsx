...
export default function StudentDashboard() {
  ...
  const [uploadingId, setUploadingId] = useState<number | null>(null) [cite: 3]
  const [expandedCourse, setExpandedCourse] = useState<number | null>(null)
  const [activeMaterial, setActiveMaterial] = useState<any>(null)
  const [sidebarOpen, setSidebarOpen] = useState(false)
  
  const [selectedProof, setSelectedProof] = useState<File | null>(null) [cite: 4]
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [resReg, resUser, resAll] = await Promise.all([
        fetch("https://backend.mejatika.com/api/registrations", { headers: { "Authorization": `Bearer ${token}` } }), [cite: 5]
      ...
      setRegistrations(Array.isArray(dataReg) ? dataReg : dataReg.data || []) [cite: 6]
      setUser(dataUser)
  ...
  // FUNGSI KHUSUS DOWNLOAD SERTIFIKAT (ANTI-UNAUTHORIZED)
  const handleDownloadCertificate = async (reg: any) => {
    const token = localStorage.getItem("token")
    if (!token) return router.push("/login")
    
    setDownloadingCertId(reg.id)
    // Sesuaikan endpoint sesuai backend: /registrations/{id}/certificate
    const downloadUrl = `https://backend.mejatika.com/api/registrations/${reg.id}/certificate`
    ...
    try {
      ...
      alert("Gagal mengunduh sertifikat. Pastikan file sudah tersedia di server.")
    } finally {
  ...
  const fetchSubmissionStatus = useCallback(async () => {
    ...
    const token = localStorage.getItem("token")
    try {
      const res = await fetch(`https://backend.mejatika.com/api/submissions/check/${activeMaterial.id}`, { [cite: 7]
      ...
      if (submission && activeStep === "tugas") {
        setStudentAnswer(submission.student_answer || "") [cite: 8]
        setTaskLink(submission.project_link || "") [cite: 9]
      }
    } catch (err) {
  ...
  const calculateProgress = (courseId: number) => { [cite: 10]
  ...
  const handleSendReply = async () => {
    if (!replyText.trim() || !submissionFeedback?.id) return [cite: 12]
    ...
    try {
      ...
      if (res.ok) { 
        setReplyText("");
        await fetchSubmissionStatus(); [cite: 13]
      }
    } catch (err) { 
  ...
  const handleSubmitTask = async () => {
    ...
    const token = localStorage.getItem("token")
    const isUpdate = submissionFeedback && submissionFeedback.id [cite: 14]
    const url = isUpdate 
      ? `https://backend.mejatika.com/api/submissions/${submissionFeedback.id}` [cite: 15]
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ 
          ...
          student_answer: studentAnswer, [cite: 16]
      ...
      if (res.ok) { 
        ...
        alert(errData.message || "Gagal mengirim tugas") [cite: 17]
      }
    } catch (err) { 
  ...
  const markStepComplete = (materialId: number, currentStep: string, nextStep: string | null) => { [cite: 18]
  ...
  const isStepLocked = (materialId: number, step: string) => {
    const p = courseProgress[materialId] || {} [cite: 19]
  ...
  const handleEnroll = async (courseId: number) => {
    ...
    try {
      ...
      const res = await fetch("https://backend.mejatika.com/api/registrations", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` }, [cite: 20]
  ...
  const handleUploadProof = async (regId: number) => {
    ...
    try {
      const token = localStorage.getItem("token") [cite: 21]
  ...
  const renderEmbed = (url: string) => {
    if (!url) return null; [cite: 23]
    let embedUrl = url
    if (url.includes("youtube.com") || url.includes("youtu.be")) {
      const videoId = url.includes("v=") ? url.split("v=")[1]?.split("&")[0] : url.split("youtu.be/")[1]?.split("?")[0] [cite: 24]
  ...
  return (
    <div className="flex min-h-screen bg-[#F8F9FB] text-zinc-900"> [cite: 25]
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 bg-zinc-950 p-4 z-[60] flex justify-between items-center">
        ...
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="text-white"> [cite: 26]
          {sidebarOpen ? <X /> : <Menu />} [cite: 27]
        </button>
      </div>

      {/* SIDEBAR */}
      <aside className={`w-72 bg-zinc-950 text-white fixed h-full flex flex-col z-50 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}> [cite: 28]
        ...
        <nav className="flex-1 px-4 mt-20 lg:mt-0 space-y-2">
          {[
            { id: "dashboard", label: "Dashboard", icon: LayoutDashboard }, [cite: 29]
        ...
        <div className="p-6 border-t border-zinc-900">
           <button onClick={() => {localStorage.clear(); router.push("/login")}} className="w-full flex items-center gap-4 px-5 py-4 text-rose-500 font-black italic uppercase text-[10px] hover:bg-rose-500/10 rounded-2xl transition-all"> [cite: 31]
      ...
      {/* MAIN CONTENT */}
      <main className={`flex-1 transition-all duration-300 ${sidebarOpen ? 'blur-sm' : ''} lg:ml-72 p-6 lg:p-10 flex flex-col mt-16 lg:mt-0`}> [cite: 32]
        ...
        {activeMenu === "dashboard" && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="bg-zinc-900 rounded-[2.5rem] lg:rounded-[3.5rem] p-8 lg:p-16 text-white relative overflow-hidden">
              ...
              <h2 className="text-4xl lg:text-6xl font-black italic uppercase tracking-tighter mb-4 leading-none">Woi, {user?.name?.split(' ')[0]}!</h2> [cite: 33]
              <p className="text-amber-500 font-bold uppercase text-[11px] tracking-[0.3em]">Siap bantai codingan hari ini?</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
              <Card className="p-8 lg:p-10 rounded-[2.5rem] bg-white border-none shadow-sm"> [cite: 34]
              ...
              <Card className="p-8 lg:p-10 rounded-[2.5rem] bg-white border-none shadow-sm">
                <p className="text-[10px] font-black uppercase text-zinc-400 mb-2">Pendaftaran</p> [cite: 35]
              ...
              <Card className="p-8 lg:p-10 rounded-[2.5rem] bg-white border-b-8 border-emerald-500 shadow-sm">
                ...
                <h3 className="text-4xl lg:text-5xl font-black italic text-emerald-600">{registrations.filter(r => r.status === 'success').length}</h3> [cite: 36]
              </Card>
            </div>

            {registrations.filter(r => r.status === 'success').length > 0 && (
              <div className="space-y-6">
                <h3 className="text-xl font-black italic uppercase tracking-tight">Lanjutkan Belajar</h3> [cite: 37]
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {registrations.filter(r => r.status === 'success').slice(0, 2).map(reg => (
                    <div key={reg.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm flex items-center justify-between group">
                      ...
                      <Button onClick={() => { setExpandedCourse(reg.course_id); setActiveMenu("materials"); }} size="icon" className="rounded-full bg-zinc-950 text-white group-hover:scale-110 transition-transform"> [cite: 39, 40]
                        <PlayCircle size={20} />
                      </Button>
                    </div>
                  ))}
        ...
        {activeMenu === "courses" && (
          <div className="space-y-10 animate-in fade-in duration-500">
            <h2 className="text-4xl font-black italic uppercase tracking-tighter">Katalog Kursus</h2> [cite: 42]
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              {availableCourses.map((course) => {
                const reg = registrations.find(r => Number(r.course_id) === Number(course.id));
                const status = reg?.status;
                return (
                  <Card key={course.id} className="rounded-[3.5rem] overflow-hidden bg-white border-none shadow-sm flex flex-col hover:shadow-xl transition-all duration-500"> [cite: 43]
                    <div className="h-48 bg-zinc-100 flex items-center justify-center relative">
                      <BookOpen className="text-zinc-300" size={60} />
                      {status === 'success' && <div className="absolute top-6 right-6 bg-emerald-500 text-white p-2 rounded-full shadow-lg"><CheckCircle2 size={20}/></div>} [cite: 44]
                    </div>
                    <CardContent className="p-10 flex-1 flex flex-col">
                      <h4 className="text-2xl font-black uppercase italic mb-6 leading-tight tracking-tighter">{course.title}</h4>
                      {status === 'success' ? ( [cite: 45]
                        <Button onClick={() => { setExpandedCourse(course.id); setActiveMenu("materials"); }} className="w-full bg-emerald-500 text-white h-14 rounded-2xl font-black italic uppercase text-[11px]">Buka Modul <Zap size={14} className="ml-2"/></Button> [cite: 46]
                      ) : status === 'pending' ? ( [cite: 47]
                        <div className="space-y-4 bg-amber-50 p-6 rounded-[2rem] border-2 border-amber-100">
                           ...
                           <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-amber-300 rounded-2xl bg-white cursor-pointer hover:bg-amber-100 transition-colors"> [cite: 48]
                           ...
                           <Button onClick={() => handleUploadProof(reg.id)} disabled={uploadingId === reg.id} className="w-full bg-zinc-950 text-amber-500 h-12 rounded-xl font-black italic uppercase text-[10px]">
                             {uploadingId === reg.id ? <Loader2 className="animate-spin" /> : "Konfirmasi Bayar"} [cite: 50]
                           </Button>
                        </div>
                      ) : (
                        <Button onClick={() => handleEnroll(course.id)} disabled={registeringId === course.id} className="w-full bg-zinc-950 text-amber-500 h-14 rounded-2xl font-black italic uppercase text-[11px]"> [cite: 51]
        ...
        {activeMenu === "materials" && ( [cite: 53]
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 animate-in fade-in duration-500">
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar"> [cite: 54]
                {registrations.filter(r => r.status === 'success').map((reg) => (
                  <div key={reg.id} className="space-y-3">
                    <button onClick={() => setExpandedCourse(expandedCourse === reg.course_id ? null : reg.course_id)} className={`w-full p-5 rounded-2xl flex items-center justify-between transition-all ${expandedCourse === reg.course_id ? 'bg-zinc-950 text-white' : 'bg-white shadow-sm'}`}> [cite: 55, 56]
                      <span className="text-[10px] font-black uppercase italic truncate pr-4">{reg.course?.title}</span>
                      <ChevronDown size={14} className={`${expandedCourse === reg.course_id ? 'rotate-180' : ''}`} /> [cite: 57]
                    </button>
                    {expandedCourse === reg.course_id && reg.course?.materials?.map((m: any) => (
                      <div key={m.id} className="ml-4 pl-4 border-l-2 border-zinc-200 space-y-2">
                        <p className="text-[9px] font-black text-amber-600 uppercase italic mb-1">{m.title}</p> [cite: 58]
                        {[
                          { id: "live", label: "Live Session", icon: Video },
                          { id: "materi", label: "Materi Pokok", icon: MonitorPlay }, [cite: 59]
                          { id: "tugas", label: "Tugas Praktik", icon: Flame },
                          { id: "feedback", label: "Feedback", icon: MessageSquare }
                        ].map((step) => {
                          const locked = isStepLocked(m.id, step.id) [cite: 60]
                          const done = courseProgress[m.id]?.[step.id]
                          return (
                            <button key={step.id} disabled={locked} onClick={() => { setActiveMaterial(m); setActiveStep(step.id); }} className={`w-full flex items-center justify-between p-3 rounded-xl text-[9px] font-black uppercase italic transition-all ${activeMaterial?.id === m.id && activeStep === step.id ? 'bg-amber-100 text-amber-700' : 'bg-white'} ${locked ? 'opacity-30 grayscale cursor-not-allowed' : 'hover:bg-zinc-50'}`}> [cite: 61, 62, 63]
                              <span className="flex items-center gap-2">{locked ? <Lock size={12} /> : <step.icon size={12} />} {step.label}</span> [cite: 64]
            ...
            <div className="lg:col-span-8">
              {activeMaterial ? ( [cite: 67]
                <div className="bg-white p-6 lg:p-12 rounded-[2.5rem] lg:rounded-[4rem] shadow-sm space-y-8 animate-in zoom-in-95 duration-500 overflow-hidden">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <h3 className="text-2xl font-black italic uppercase flex items-center gap-3">
                      <span className="h-8 w-8 bg-amber-500 rounded-lg flex items-center justify-center text-zinc-950 text-xs">0{activeStep === "live" ? "1" : activeStep === "materi" ? "2" : activeStep === "tugas" ? "3" : "4"}</span> [cite: 68]
                      {activeStep === "live" ? "Live" : activeStep === "materi" ? "Materi" : activeStep === "tugas" ? "Tugas" : "Review"}
                    </h3>
                    <div className="px-4 py-1.5 bg-zinc-100 rounded-full text-[9px] font-black uppercase text-zinc-400 italic truncate max-w-[200px]">{activeMaterial.title}</div> [cite: 69]
                  </div>

                  {activeStep === "live" && (
                    <div className="space-y-8"> [cite: 70]
                  ...
                  {activeStep === "materi" && (
                    <div className="space-y-8"> [cite: 71]
                      {activeMaterial.content && !activeMaterial.content.includes('<iframe') && renderEmbed(activeMaterial.file)}
                      <div className="bg-zinc-50 rounded-[2.5rem] p-8 border border-zinc-100 prose prose-zinc max-w-full italic font-medium"> [cite: 72]
                        <div dangerouslySetInnerHTML={{ __html: activeMaterial.content }} style={{ wordBreak: 'break-word' }} />
                      </div>
                      <Button onClick={() => markStepComplete(activeMaterial.id, "materi", "tugas")} className="w-full bg-emerald-500 text-white h-16 rounded-[2rem] font-black italic uppercase text-[11px]">Sudah Paham, Ke Tugas</Button> [cite: 73]
                    </div>
                  )}

                  {activeStep === "tugas" && (
                    <div className="space-y-6"> [cite: 74]
                       <div className="p-8 bg-amber-50 rounded-[2.5rem] border-2 border-amber-100">
                          ...
                          <div className="text-sm italic font-medium text-zinc-800">{activeMaterial.quiz_task || "Silahkan kerjakan tugas sesuai arahan di video materi."}</div> [cite: 75, 76]
                       </div>
                       
                       {submissionFeedback && ( [cite: 77]
                         <div className="flex items-center gap-2 px-6 py-2 bg-emerald-50 text-emerald-700 rounded-full text-[10px] font-black italic uppercase w-fit">
                           <RefreshCw size={12} className="animate-spin-slow" /> Anda sedang mengedit tugas yang sudah dikirim [cite: 78]
                        </div>
                       )}

                       <textarea 
                        value={studentAnswer} 
                        onChange={(e) => setStudentAnswer(e.target.value)} [cite: 79]
                       ...
                       <input 
                        type="text" 
                        value={taskLink} [cite: 80]
                        onChange={(e) => setTaskLink(e.target.value)} [cite: 81]
                       ...
                       <Button onClick={handleSubmitTask} disabled={isSubmittingTask} className={`w-full h-16 rounded-[2rem] font-black italic uppercase text-[11px] ${submissionFeedback ? 'bg-emerald-600 text-white' : 'bg-zinc-950 text-amber-500'}`}> [cite: 83]
                         {isSubmittingTask ? <Loader2 className="animate-spin" /> : (submissionFeedback ? "Update Tugas Anda" : "Submit Tugas Anda")} [cite: 84]
                       </Button>
                    </div>
                  )}

                  {activeStep === "feedback" && ( [cite: 85]
                    <div className="space-y-6 animate-in fade-in">
                       {submissionFeedback ? (
                         <div className="space-y-6">
                           <div className="bg-zinc-900 p-8 rounded-[3rem] text-white"> [cite: 86]
                               <div className="flex justify-between items-start mb-6">
                                  <div className="flex items-center gap-3">
                                     <div className="h-10 w-10 bg-amber-500 rounded-full flex items-center justify-center text-zinc-950"><UserCircle2 size={24}/></div> [cite: 87]
                                     <div>
                                        <p className="text-[8px] font-black text-amber-500 uppercase tracking-widest">Mentor Review</p> [cite: 88]
                                        <h4 className="text-xl font-black italic uppercase">Feedback</h4>
                                     </div> [cite: 89]
                                  </div>
                                  <div className="text-center bg-zinc-800 px-4 py-2 rounded-2xl">
                                     <p className="text-[8px] font-black text-zinc-500 uppercase">Nilai</p> [cite: 90]
                                     <div className="text-3xl font-black italic text-amber-500">{submissionFeedback.score || "—"}</div> [cite: 91]
                                  </div>
                               </div>
                               <div className="p-6 bg-zinc-800/50 rounded-2xl italic text-zinc-300 text-sm"> [cite: 92]
                                  "{submissionFeedback.mentor_feedback || "Menunggu review mentor..."}"
                               </div>
                            </div>

                            <Button 
                              variant="outline" [cite: 93]
                              onClick={() => setActiveStep("tugas")} [cite: 94]
                              className="w-full border-2 border-zinc-200 rounded-2xl font-black italic uppercase text-[10px] h-12 hover:bg-zinc-50"
                            >
                               <RefreshCw size={14} className="mr-2" /> Perbaiki / Update Tugas [cite: 95]
                            </Button>

                            <div className="bg-zinc-50 p-6 rounded-[2.5rem] border border-zinc-100 space-y-4"> [cite: 96]
                               {submissionFeedback.student_reply && (
                                 <div className="flex flex-col items-end">
                                    <div className="bg-amber-100 p-4 rounded-2xl text-sm italic font-medium max-w-[90%]">{submissionFeedback.student_reply}</div> [cite: 97]
                                    <span className="text-[8px] font-black uppercase text-zinc-400 mt-1">Balasan Anda</span>
                                 </div> [cite: 98]
                               )}
                               {!submissionFeedback.student_reply && (
                                 <div className="relative"> [cite: 99]
                                    ...
                                    <button onClick={handleSendReply} className="absolute bottom-4 right-4 h-10 w-10 bg-zinc-950 text-amber-500 rounded-full flex items-center justify-center"><Send size={16}/></button> [cite: 100]
                                 </div>
                               )}
                            </div> [cite: 101]
                         </div>
                         ...
                         <div className="bg-emerald-50 p-12 rounded-[3rem] text-center border-2 border-emerald-100">
                            <CheckCircle2 className="mx-auto text-emerald-500 mb-4" size={40} /> [cite: 102]
                ...
                <div className="h-[60vh] flex flex-col items-center justify-center text-zinc-200 border-4 border-dashed border-zinc-100 rounded-[3rem] lg:rounded-[5rem] bg-white/20">
                  <PlayCircle size={80} className="opacity-10 mb-4" /> [cite: 105]
        ...
        {activeMenu === "certificates" && ( [cite: 106]
          <div className="space-y-10 animate-in fade-in duration-500">
            <h2 className="text-4xl font-black italic uppercase tracking-tighter">Sertifikat</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {registrations.filter(r => r.status === 'success').map(reg => {
                const isFinished = calculateProgress(reg.course_id) === 100; [cite: 107]
                ...
                return (
                  <Card key={reg.id} className={`rounded-[2.5rem] p-8 flex flex-col items-center text-center ${isFinished ? 'bg-white' : 'bg-zinc-50 opacity-50'}`}>
                    ...
                    <Button 
                      ...
                      className={`w-full h-12 rounded-xl font-black italic uppercase text-[10px] ${isFinished ? 'bg-zinc-950 text-amber-500' : 'bg-zinc-200'}`} [cite: 109]
                    >
                      {isDownloading ? <Loader2 className="animate-spin" /> : (isFinished ? "Download PDF" : "Progress: " + calculateProgress(reg.course_id) + "%")}
                    </Button>
                  </Card>
                )
              })} [cite: 110]
        ...
        <footer className="py-12 border-t border-zinc-100 mt-auto text-center"> [cite: 111]
