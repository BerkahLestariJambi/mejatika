...
} from "lucide-react" // SUDAH FIXED: lucide-react
          ...
          <div className="flex-1">
              ...
              <div className="space-y-10 animate-in fade-in">
                  <div className="relative overflow-hidden bg-zinc-900 rounded-[3.5rem] p-12 text-white shadow-2xl">
                    <div className="relative z-10">
                        ...
                        <p className="text-zinc-400 font-bold uppercase text-xs tracking-widest">Ayo lanjut belajar lagi.</p>
                    </div>
            ...
            {activeMenu === "courses" && (
              <div className="space-y-8 animate-in fade-in">
                <h2 className="text-3xl font-black italic uppercase tracking-tighter">Pilihan Kursus</h2>
                        ...
                        <CardContent className="p-8">
                          ...
                          {status === 'success' ? (
                            ...
                            <div className="w-full bg-amber-50 border border-amber-200 text-amber-700 rounded-2xl p-4 text-center">
                              <p className="text-[9px] font-black uppercase italic leading-tight">Menunggu Pembayaran (Max 3 Hari)</p>
                            </div>
                          ) : (
                            <div className="space-y-3">
                              <p className="text-[10px] font-black text-zinc-400 uppercase italic text-center">Belum terdaftar</p>
            ...
            {activeMenu === "materials" && (
              <div className="grid grid-cols-12 gap-8 animate-in fade-in">
                <div className="col-span-4 space-y-6">
                  <h2 className="text-2xl font-black italic uppercase tracking-tighter">Materi</h2>
                  <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                    {registrations.filter(r => r.status === 'success').map((reg) => (
                      <div key={reg.id} className="space-y-3">
                        <button onClick={() => setExpandedCourse(expandedCourse === reg.id ? null : reg.id)} className={`w-full p-4 rounded-2xl flex items-center justify-between ${expandedCourse === reg.id ? 'bg-zinc-900 text-white' : 'bg-white shadow-sm'}`}>
                        ...
                        {expandedCourse === reg.id && reg.course?.materials?.map((m: any, idx: number) => (
                          <button key={m.id} onClick={() => setActiveMaterial(m)} className={`w-full p-4 rounded-2xl text-left border-2 flex items-center gap-3 ${activeMaterial?.id === m.id ? 'border-amber-500 bg-white' : 'border-transparent bg-white shadow-sm'}`}>
                ...
                <div className="col-span-8">
                  {activeMaterial ? (
                    <div className="space-y-6 animate-in fade-in">
                      <div className="bg-zinc-950 rounded-[2.5rem] p-6">{renderPreview(activeMaterial.file)}</div>
                      <Card className="p-10 bg-white rounded-[2.5rem] border-none">
                      ...
                      {completedStatus[activeMaterial.id] && (
                        <Card className="p-8 bg-white border-2 border-amber-100 rounded-[2.5rem] shadow-xl">
                          ...
                          {submittedTasks[activeMaterial.id] ? (
                            ...
                            <div className="space-y-4">
                              <textarea id="taskInput" placeholder="Link Drive..." className="w-full h-32 rounded-3xl p-6 bg-zinc-50 outline-none border border-zinc-100" />
                              <Button onClick={() => { const val = (document.getElementById('taskInput') as HTMLTextAreaElement).value; if(val) setSubmittedTasks({...submittedTasks, [activeMaterial.id]: val}); }} className="w-full bg-zinc-950 text-amber-500 rounded-2xl font-black h-14 uppercase italic text-[11px]">Kirim Tugas</Button>
                            </div>
                          )}
                        </Card>
                      )}
            ...
            {activeMenu === "certificates" && (
              <div className="h-[60vh] flex flex-col items-center justify-center">
                <Award size={64} className="text-amber-500 mb-6" />
                <h2 className="text-4xl font-black italic uppercase">Belum Ada Sertifikat</h2>
              </div>
            )}
          </div>

          <footer className="py-10 border-t border-zinc-100 mt-20">
            <div className="flex justify-between items-center text-[10px] font-black uppercase italic text-zinc-400">
