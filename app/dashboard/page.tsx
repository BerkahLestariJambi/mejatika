...
export default function StudentDashboard() {
  ...
  // Menghitung total materi yang sudah selesai di semua kursus
  ...
  return (
    <div className="flex min-h-screen bg-[#F8F9FB] text-zinc-900">
      ...
      <main className="flex-1 ml-72 p-10 max-w-7xl">
        ...
        {/* VIEW: SERTIFIKAT (DENGAN LOGIKA CEK 12 MATERI) */}
        {activeMenu === "certificates" && (
          <div className="space-y-10 animate-in fade-in">
            {totalCompleted < 12 ? (
              /* PESAN JIKA BELUM SELESAI 12 MATERI */
              <div className="relative overflow-hidden bg-zinc-900 rounded-[3.5rem] p-12 text-white shadow-2xl">
                <div className="relative z-10">
                  <h2 className="text-5xl font-black italic uppercase tracking-tighter mb-4">Halo, {user?.name}!</h2>
                  ...
                  <Button 
                    ...
                    className="mt-8 bg-amber-500 text-zinc-950 rounded-2xl font-black italic uppercase text-[10px] h-12 px-8 hover:bg-white transition-colors"
                  >
              ...
              /* TAMPILAN JIKA SUDAH LULUS */
              <>
                <h2 className="text-3xl font-black italic uppercase tracking-tighter">Sertifikat Kelulusan</h2>
                    ...
                    <Card key={reg.id} className="border-none shadow-sm rounded-[2.5rem] p-8 bg-white flex items-center justify-between border-b-4 border-emerald-500">
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Status: Kursus Selesai</p>
                        </div>
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {registrations.map((reg) => (
                <Card key={reg.id} className="group border-none shadow-sm rounded-[2.5rem] overflow-hidden bg-white hover:shadow-2xl transition-all duration-500">
                  <div className="h-32 bg-zinc-100 flex items-center justify-center group-hover:bg-amber-50 transition-colors">
                  ...
                  <CardContent className="p-8">
                    ...
                    <Button 
                      ...
                      className="w-full bg-zinc-950 text-amber-500 rounded-2xl font-black italic uppercase text-[10px] h-12 hover:bg-amber-500 hover:text-zinc-950"
                    >
                          ...
                          <div key={m.id}>
                            <button 
                              ...
                              className={`w-full p-4 rounded-2xl text-left border-2 transition-all flex items-center gap-3 ${
                                activeMaterial?.id === m.id ? 'border-amber-500 bg-white shadow-lg' : 'border-transparent bg-white shadow-sm'
                              }`}
                            >
                              <div className={`h-8 w-8 rounded-lg flex items-center justify-center text-[10px] font-black italic ${activeMaterial?.id === m.id ? 'bg-amber-500 text-zinc-950' : 'bg-zinc-100 text-zinc-400'}`}>
                                {idx + 1}
                              </div>
