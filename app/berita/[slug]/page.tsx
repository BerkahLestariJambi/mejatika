...
import { Calendar, User, ArrowLeft, Quote, Share2, MessageCircle, Facebook, ImageIcon } from "lucide-react"
import { Button } from "@/components/ui/button"
import Link from "next/link"

export default function NewsDetailPage({ article, onBack }: { article: any, onBack: () => void }) {
  
  // URL untuk share (Aman untuk Client Side)
  const shareUrl = typeof window !== 'undefined' ? window.location.href : '';

  // 1. Validasi Awal: Jika data article tidak ada, tampilkan loading/pesan error sederhana
  if (!article) return <div className="p-10 text-center font-bold">Memuat data warta...</div>;
    ...
    <article className="mt-6 flex flex-col items-center animate-in fade-in slide-in-from-bottom-10 duration-700">
      ...
      <div className="w-full max-w-[92%] lg:max-w-[850px] bg-[#fffdfa] shadow-2xl px-8 lg:px-20 py-16 -mt-8 relative border-x border-black/5 z-20">
        <header className="space-y-4 text-center mb-10">
          {/* 2. Safe Category: Menggunakan optional chaining (?.) untuk cegah error */}
        ...
        {/* 3. Safe Image: Cek apakah URL gambar tersedia */}
        <div className="relative w-full aspect-video rounded-lg overflow-hidden shadow-lg border border-black/5 mb-12 bg-zinc-100 flex items-center justify-center">
          {article?.image ? (
        ...
        {/* 4. Render HTML: Mengatasi tag <p> dan &nbsp; agar tampil visual */}
        ...
        {/* 5. Safe Quote: Hanya tampil jika data ada */}
