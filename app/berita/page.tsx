...
export default function BeritaPage() {
  ...
  // Membersihkan tag HTML untuk tampilan ringkasan
    ...
    <div className="min-h-screen bg-zinc-50 flex flex-col">
      ...
      <div className="bg-amber-500 py-16 px-4 relative overflow-hidden">
        ...
        <div className="container mx-auto text-center relative z-10">
          ...
          <p className="text-amber-900 font-bold mt-2 uppercase tracking-widest text-xs">Update informasi terbaru dari MEJATIKA</p>
        </div>
      </div>
      ...
      <main className="flex-grow container mx-auto px-4 py-12 -mt-10">
        {loadingList ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {news.map((item: any) => (
              <Card key={item.id} className="group overflow-hidden flex flex-col shadow-xl border-none bg-white rounded-[2rem]">
                ...
                <CardContent className="p-7 flex flex-col flex-grow">
                  ...
                  <Link href={`/berita/${item?.slug}`} className="w-full">
                    <button className="w-full flex items-center justify-center gap-2 bg-amber-500 hover:bg-zinc-900 text-white font-black uppercase text-[10px] tracking-widest py-4 rounded-2xl transition-all shadow-lg">
