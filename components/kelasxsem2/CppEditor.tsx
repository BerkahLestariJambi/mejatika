...
export default function CppEditor() {
  ...
  useEffect(() => { 
    setMounted(true); 
    setTerminalLines([{ content: "Terminal Siap. Klik Run." }]);
  }, []);
  ...
  const handleAutoRun = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      // Tambahkan angka yang diketik ke baris prompt
      ...
      try {
        ...
        // Ambil baris terakhir saja (Hasilnya adalah = X)
        const result = lines.length > 1 ? lines[lines.length - 1] : "";
        if (result) setTerminalLines(prev => [...prev, { content: result }]);
        
      } catch (err) {
        setTerminalLines(prev => [...prev, { content: "Error: Gagal koneksi." }]);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <div className="flex flex-col rounded-xl overflow-hidden border border-zinc-800 bg-[#1e1e1e]">
      <div className="flex items-center justify-between bg-[#252526] px-4 py-2 border-b border-zinc-800">
        <div className="flex items-center gap-2">
          <Code2 size={16} className="text-blue-400" />
          <span className="text-xs font-mono text-zinc-300 tracking-tight">main.cpp</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 h-[500px]">
        <div className="border-r border-zinc-800">
          <Editor height="100%" defaultLanguage="cpp" theme="vs-dark" value={code} onChange={(v) => setCode(v || "")} options={{ minimap: { enabled: false }, fontSize: 14, automaticLayout: true }} />
        </div>

        <div className="bg-black p-5 font-mono text-sm overflow-y-auto flex flex-col">
